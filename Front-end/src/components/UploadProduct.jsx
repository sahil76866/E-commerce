...
export default function UploadProduct({ onclose, fetchdata }) {
  ...
  const handleUploadProduct = async (e) => {
    ...
    try {
      ...
      const newProductImages = uploadResults.map((result) => result.url);
      setData((prev) => ({ ...prev, ProductImage: [...prev.ProductImage, ...newProductImages] }));
    } catch (error) {
      console.error(error);
    }
  };
    ...
    <div className='fixed w-full h-full top-0 right-0 left-0 bottom-0  bg-slate-300 bg-opacity-35 flex justify-center items-center'>
      <div className="bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden">
        ...
        <form action="" onSubmit={handleSubmit} className='grid p-4 gap-2 overflow-y-scroll h-full pb-9'>
          ...
          <button className='px-3 py-2 bg-red-500 text-white border rounded-xl m-5  hover:bg-red-700'>Upload Product</button>
        </form>
